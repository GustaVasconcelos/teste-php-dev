...
@section('title', 'Categorias | Sub Categorias | Edição')
...
<div class="container-fluid">
    <div class="text-xs font-weight-bold text-primary text-uppercase mb-1 text-dark">
        Categorias <i class="fas fa-chevron-right mx-1"></i> Sub Categorias <i class="fas fa-chevron-right mx-1"></i> Edição
    </div>
    ...
    <div class="p-3 mt-2 rounded shadow-sm mb-2 bg-white">
        ...
        <form action="{{ route('categories.subcategories.update', [$categoryId, $subCategory->id]) }}" method="post" class="row" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div class="col-12 mb-3">
                <label for="name" class="form-label text-dark font-weight-bold mt-2">Nome:</label>
                <input type="text" id="name" name="name" value="{{ old('name', $subCategory->name) }}" class="form-control" placeholder="Digite o nome da sub categoria">
            </div>
